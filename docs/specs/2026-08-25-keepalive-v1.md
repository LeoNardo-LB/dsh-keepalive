...
- 2026-08-26：提供商面板模型重做（用户反馈）：面板恒展示**全部已注册路由**（status 新增 availableProviders，随 5s 轮询跟进新路由），config 降级为每家覆盖项（缺席=未参与，一键参与/禁用/编辑模型）；移除走 settings mutate 的 path-op unset（实测 settings.update 为递归深合并，patch 永远无法删 key——删除是它唯一无法表达的语义）。新增 e2e s8 场景（可见→参与→调度→发帖全链路）与 routes 单测 7 例。
