...
- [~] **#2 dsh-keepalive 插件 v1** `feat`
  - 按 21 问共识实现双半边保活插件（调度/静默发送/持久化/HTTP 面/面板），Docker 实机验证通过后验收
  - → 设计：docs/specs/2026-08-25-keepalive-v1.md
  - 自动化全绿（单测 36/36 + Docker E2E 7/7 + 重启补发 1/1）；待用户浏览器人工验证时间性现象后验收
