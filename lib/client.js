...
var P = (() => {
  try {
    ...
    const messageText = mod.MessageText !== void 0 ? pick("MessageText") : (props) => (0, import_react.createElement)(markdownText, {
      text: props.text,
...
function ConfigForm(props) {
  ...
  const dirty = interval !== String(config.intervalMinutes) || jitter !== String(config.jitterPercent) || threshold !== String(config.autoPause.threshold);
  ...
  const valid = Number.isFinite(parsed.interval) && parsed.interval >= 1 && Number.isFinite(parsed.jitter) && parsed.jitter >= 0 && parsed.jitter <= 100 && Number.isFinite(parsed.threshold) && parsed.threshold >= 1;
...
function ProviderCard(props) {
  ...
  const [modelChoice, setModelChoice] = (0, import_react3.useState)(String(configuredModel));
  const [modelMenuOpen, setModelMenuOpen] = (0, import_react3.useState)(false);
...
function HistorySection(props) {
  ...
  const [statsOpen, setStatsOpen] = (0, import_react4.useState)(false);
  const [openAt, setOpenAt] = (0, import_react4.useState)(null);
